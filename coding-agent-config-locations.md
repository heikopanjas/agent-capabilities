...
Instruction files, custom prompts, skills & subagents for CLI / agentic coding tools — September 2026
...
| **Settings** ★ | `~/.claude/settings.json`; Managed: `/etc/claude-code/managed-settings.json` + `managed-settings.d/*.json` (Linux/WSL) · `/Library/Application Support/ClaudeCode/managed-settings.json` + `managed-settings.d/` + MDM plist `com.anthropic.claudecode` (macOS) · `C:\Program Files\ClaudeCode\managed-settings.json` + `managed-settings.d\` + `HKLM\SOFTWARE\Policies\ClaudeCode` + `HKCU\SOFTWARE\Policies\ClaudeCode` (Windows; legacy `C:\ProgramData\ClaudeCode\` no longer read — exact deprecation version unconfirmed by current docs) | `.claude/settings.json`, `.claude/settings.local.json` (gitignored). Precedence: Managed > CLI args > Local > Project > User |
| **Skills** ★ | `~/.claude/skills/*/SKILL.md`; skills synced from claude.ai land at `~/.claude/skills/synced/<name>/SKILL.md`, invoked as `/anthropic-skills:<name>` (terminal syncing v2.1.273+) | `.claude/skills/*/SKILL.md` — progressive disclosure per agentskills.io |
| **Subagents** ★ | `~/.claude/agents/*.md`; managed/enterprise subagents deployed inside the managed-settings directory take highest priority | `.claude/agents/*.md` — Markdown + YAML frontmatter; subdirectory discovery supported, but identity comes only from the `name` field — no directory-qualified naming on clash (that pattern, e.g. `apps/web:deploy`, applies to skills, not subagents); same-name files within one `.claude/agents/` tree resolve by unspecified filesystem read order, but across nested project `.claude/agents/` directories the definition closest to the working directory wins (v2.1.178+); only plugin subagents get scoped identifiers like `my-plugin:review:security`. `isolation: worktree` frontmatter runs the subagent in a temporary git worktree; `background` field; `omitClaudeMd` field (v2.1.271+) skips injecting CLAUDE.md; other frontmatter fields include `effort`, `color`, `initialPrompt`, `experimental`; `--agents` CLI flag defines session-scoped JSON subagents; nested spawning up to depth 3 by default (v2.1.219+; was depth 5 in v2.1.172–v2.1.216, depth 1 in v2.1.217–v2.1.218) |
| **Agent memory** ◆ | `~/.claude/agent-memory/<agent-name>/` — subagent user-scoped (`memory: user`). Main-session auto-memory: `~/.claude/projects/<project>/memory/MEMORY.md` (v2.1.59+; `autoMemoryDirectory` setting overrides). `CLAUDE_CODE_PROJECT_DIR_NAME` (paired with `CLAUDE_CONFIG_DIR`, v2.1.234+) lets multiple checkouts of the same repo share one auto-memory directory | `.claude/agent-memory/<agent-name>/` (`memory: project`, committable), `.claude/agent-memory-local/<agent-name>/` (`memory: local`, gitignored) |
| **Commands** ★ | `~/.claude/commands/*.md` → `/<name>` (superseded by skills, still works) | `.claude/commands/*.md` → `/<name>` (superseded by skills, still works) ⁴. `/import` (v2.1.213+) can pull `AGENTS.md` and other agents' configs (MCP servers, commands, subagents, skills) into Claude Code, as an alternative to the `@AGENTS.md` import trick |
| **Workflows** ◆ | `~/.claude/workflows/*.js` — JavaScript multi-agent orchestration scripts | `.claude/workflows/*.js` — each saved file becomes a `/<name>` command; `/workflows` opens the separate run-management/progress view (list, watch, pause, stop). On the Pro plan, dynamic workflows require manual enablement via `/config`; sub-features are version-gated (size guideline v2.1.202+, `--effort ultracode` v2.1.203+, `/workflow-authoring` skill v2.1.248+, usage-limit pause v2.1.271+) |
| **Output styles** ◆ | `~/.claude/output-styles/*.md` — custom response format styles (frontmatter-based: `name`, `description`, `keep-coding-instructions`) | `.claude/output-styles/*.md`. Standalone `/output-style` command deprecated v2.1.73, removed v2.1.91 — set via `/config` or the `outputStyle` setting key. Switching now applies starting the next message rather than requiring `/clear`/a new session (changed v2.1.251) |
...
> **Rust rewrite complete:** The TypeScript CLI was gradually phased out through late 2025 (no formal retirement date announced). The current codebase is ~96.5–96.7% Rust (`codex-rs/`, a continuously drifting figure). The Rust rewrite began around June 2025 and reached production parity/stability around late 2025 (coinciding with the GPT-5 family launch); precise GA date unconfirmed by official sources. Release cadence remains rapid, with alpha builds shipping multiple times per week (e.g. `rust-v0.155.0-alpha.16` as of 2026-09-17). Core config paths are stable post-transition. (A separate TypeScript **SDK**, `sdk/typescript`, still exists and is unrelated to the retired TS CLI.)
...
| Feature | Global (user) | Project (repo) |
|---|---|---|
| **Instructions** ★ | `$HOME/.copilot/copilot-instructions.md` (CLI). VS Code: `~/.copilot/instructions/*.instructions.md`, `~/.claude/rules` — configurable via `chat.instructionsFilesLocations` (now **deprecated**: docs say it's used only by the Local agent, migrate customizations to supported locations). `$COPILOT_CUSTOM_INSTRUCTIONS_DIRS` | `.github/copilot-instructions.md` — repository-wide. Also reads `AGENTS.md` (**treated as primary instructions**; CLI + VS Code via `chat.useAgentsMdFile`; experimental nested: `chat.useNestedAgentsMdFiles`), `CLAUDE.md`, `GEMINI.md` at root. VS Code additionally reads `.claude/CLAUDE.md` and `CLAUDE.local.md` (workspace) and `~/.claude/CLAUDE.md` (user home) via `chat.useClaudeMdFile`. VS Code also reads a project-level `.claude/rules/` folder (uses a `paths` key instead of `applyTo`) |
...
| **MCP** ◆ | `~/.copilot/mcp-config.json` — CLI, `mcpServers` key. VS Code: user-profile `mcp.json` via "MCP: Open User Configuration" command, `servers` key | `.vscode/mcp.json` — VS Code project-level MCP, `servers` key (no longer read by Copilot CLI as of v1.0.22). Copilot CLI: `.mcp.json` (project root/cwd, shipped v1.0.22, 2026-04-09) and auto-loaded `.github/mcp.json` (shipped v1.0.61, 2026-06-09), `mcpServers` key — CLI walks cwd → git root loading every match found, closest wins ([github/copilot-cli#2528](https://github.com/github/copilot-cli/issues/2528), closed/shipped); when both files exist in the same directory, `.mcp.json` takes precedence over `.github/mcp.json`. Cloud agent's repo-level MCP config is entered separately as JSON directly via the repo's Settings → Copilot → MCP servers page on GitHub.com — not a committed file. Key name diverges by surface: CLI uses `mcpServers`, VS Code uses `servers` |
...
| Feature | Global (user) | Project (repo) |
|---|---|---|
| **Instructions** ★ | User Rules (Customize → Rules) — plain text; always applied. Unified "Customize" page (introduced v3.9, Jun 22, 2026) centrally manages plugins/rules/skills/MCP/subagents/commands/hooks at user/team/workspace scope; v3.11 (Jul 10, 2026 — side chats, conversation search, expanded cloud-agent hooks) remains the newest numbered release as of Sep 2026, but frequent unversioned feature drops have continued: Slack integration improvements (Jul 17), Cursor Router auto-mode model routing (Jul 22), Cursor Start pricing tier for India (Jul 28), Cursor for iPad (Jul 29), Google Workspace Plugins — Gmail/Drive/Calendar access for agents (Aug 3), Grok 4.6 model (Aug 12), Origin code-hosting beta (Aug 17), Cloud Agents/Cursor Harness improvements — `/goal` command, PR/Slack subscriptions, isolated-VM subagents (Aug 19), "Start from Scratch" prompt-without-a-repo (Aug 27), Self-hosted Machines — tool execution kept in customer infra, introduces a `CURSOR_CONFIG_DIR` env var override for the CLI config dir (Sep 2), Cursor Projects — a coordinator agent delegating to many subagents, cloud-resident with cross-session shared context, but no new `.cursor/` config path (Sep 10); no new numbered release found through Sep 17, 2026 | `<repo>/AGENTS.md` — root level and subdirectories; plain-markdown alternative to `.cursor/rules`. Legacy: `.cursorrules` (deprecated since ~v0.43; officially deprecated and undocumented but still functional as of Sep 2026 — migrate to `.cursor/rules/`) |
...
> **Note:** Current version: 2.25.4 (September 12, 2026). Core config paths stable since Vibe 2.0, though several features were added after: AGENTS.md parent-folder walking (2.5.0), `.agents/skills/` discovery (2.2.0), `~/.agents/skills` (2.11.0), hooks introduced (2.9.0) then breaking-changed (2.15.0), project-config persistence (2.18.3), hooks graduated from experimental to stable with renamed events (2.21.0, 2026-07-17), full migration to a `ConfigOrchestrator` for config handling (2.22.0), managed-shell/app-server refactor (2.23.0), built-in skill-creator skill (2.23.2), `/retry` and fork-vs-rewind-in-place `/rewind` (2.23.3, 2026-08-03), admin config layer for shared/enforced config overlaying user config (2.24.0, 2026-08-05, no new documented file path), inline ghost-text skill completion and faster resume via deferred sub-agent instantiation (2.24.1, 2026-08-11), in-app session picker/`/log-level`/worktree cleanup/admin-config retry improvements (2.24.2, 2026-08-18), process renamed "Vibe CLI" with PID in status bar plus live slash-command/setting pickers during agent runs (2.24.3, 2026-08-20), LLM-generated session titles/queued-message edit mode/git info in session header/reused model connections/fixed Nix-managed skills symlink loading (2.24.4, 2026-08-26), fuzzy search in the MCP servers/connectors browser plus faster startup via lazy imports (2.24.5, 2026-08-27), `/skills` browser with search/filter, plugin-declared MCP servers surfaced in `/mcp`, connectors (Notion, Linear, Slack, GitHub, Gmail, Calendar, Drive) integrated into ACP, and session-scoped model pinning for resumed conversations (2.25.0, 2026-09-03/04), a worktree plugin managing git worktrees under `$VIBE_HOME/worktrees` (retains up to 15 by default, protects active/dirty/unpushed trees) plus a new `session_logging.generate_titles` config key — off by default, disabling auto-generated background LLM session titles (2.25.1, 2026-09-09), incremental releases with no notable path/config changes (2.25.2–2.25.4, 2026-09-10 to 2026-09-12). None of this changes documented config paths.
...
>
> **V2 beta:** A beta docs tree (`opencode.ai/v2/docs/`) exists alongside stable V1 as of September 2026, with a migration guide at [`opencode.ai/v2/docs/migrate-v1`](https://opencode.ai/v2/docs/migrate-v1/), which states "OpenCode 1 and OpenCode 2 both use the `opencode` command and are no longer installed side by side by default" — the earlier separate `opencode2` binary naming is being phased out (some third-party/beta-channel builds still reference it). V2 drops `CLAUDE.md` instruction-file discovery (AGENTS.md only) and consolidates `tui.json` into `cli.json` (`~/.config/opencode/cli.json`), but does **not** drop Claude/agents skill compat as previously thought — V2 still searches `~/.claude/skills`, `~/.agents/skills`, `.claude/skills`, `.agents/skills` alongside `.opencode/skills`, and additionally discovers from both `.opencode/skill/` (singular) and `.opencode/skills/` (plural). V1 remains the stable release (latest v1.18.31, 2026-09-14; no V2 banner on the install page). The table below documents current stable V1 behavior only.
...
> **Philosophy:** Pi is a minimal terminal harness — "primitives, not features." It deliberately ships **no built-in MCP, subagents, or plan mode**; those are added as TypeScript extensions (the README states this as a core design choice and no longer names `mcp.json` as even a proposed convention). Distributed on npm as `@earendil-works/pi-coding-agent` (migrated from `@mariozechner/pi-coding-agent` at v0.74.0 — old package deprecated at v0.73.1, new repo at `earendil-works/pi`). Current version: 0.85.1 (npm, released 2026-09-05; intermediate releases 0.84.4 on 2026-08-28 and 0.85.0 on 2026-09-04 added `SessionManager.inMemory()` restorable sessions, persistent Claude thinking-effort selection, and a GPT-6 Astra model). Core paths stable since the v0.74.0 rename; `trust.json` added v0.79.0.
...
| **Settings** ◆ | `~/.pi/agent/settings.json` — includes `defaultTools` (v0.84.2+, configures initial built-in tool selection globally or per-project) and `defaultProjectTrust` (`ask`/`never`/`always`, controls trust fallback for non-interactive modes `-p`/`--mode json`/`--mode rpc`); `~/.pi/agent/keybindings.json` — keyboard shortcuts; `~/.pi/agent/trust.json` — saved project trust decisions (v0.79.0) | `.pi/settings.json` — project overrides global |
...
| **MCP / Subagents** ○ | Not built in — provided via extensions ("no MCP, no subagents" stated as a core design choice). `~/.pi/agent/mcp.json` was previously floated as a proposed community extension convention, but the README no longer names it at all (unshipped; native MCP/ACP support is still only under discussion) | `.pi/mcp.json` — previously proposed as a project-level convention distinct from the global path; no longer referenced in current docs |
...
| **Skills** ◆ | `~/.cline/skills/*/SKILL.md` | `.cline/skills/*/SKILL.md` (recommended), `.clinerules/skills/`, `.claude/skills/`; progressive disclosure (shipped v3.48.0, Jan 2026). Made always-on with the Settings → Features → Enable Skills toggle removed entirely in a single v3.57.0 change (~2026-02-05) — skills are now always on, with only per-skill toggles. (v3.56.0, ~2026-01-30, was unrelated to skills — it shipped GPT-5 OAuth and Jupyter notebook support.) A v4.x line (Customize marketplace) has since shipped, current v4.1.17 (~2026-09-02); Cline now versions Desktop (v0.0.30), CLI (v3.0.62), and SDK (v0.0.83) as separate release tracks (all ~2026-09-15/17) |
| **Agents** ◆ | `~/.cline/agents/` | `.cline/agents/` |
| **Hooks** ◆ | `~/.cline/hooks/`; compatibility `~/Documents/Cline/Hooks/` | `.cline/hooks/` and `.clinerules/hooks/`; lifecycle scripts receive/return JSON. The `/customization/hooks` doc page has been restructured to point to "SDK Plugins" (`/sdk/plugins`) instead of listing concrete paths, which no longer explicitly corroborates `.clinerules/hooks/` — treat that specific path with slightly lower confidence |
...
| **MCP** ◆ | `~/.cline/data/settings/cline_mcp_settings.json` (honors `CLINE_DATA_DIR`; also overridable via undocumented `CLINE_MCP_SETTINGS_PATH`) is the confirmed real runtime path; the CLI-specific MCP doc's `~/.cline/mcp.json` is a documented bug, not a real alternate path (open issue [cline/cline#11671](https://github.com/cline/cline/issues/11671)) | Managed through Cline settings; no separate committed project MCP filename documented |
...
| **Skills** ◆ | — | `.agents/skills/*/SKILL.md` (recommended); also discovered: `.devin/skills/`, `.github/skills/`, `.claude/skills/`, `.cognition/skills/`, `.windsurf/skills/` — six paths scanned in every repo (docs no longer list `.cursor/skills/`, `.codex/skills/`, or `.codeium/skills/`); discovered automatically or invoked with `@skills:<name>`; only one skill can be active at a time — a new invocation replaces the prior one |
...
| **MCP** ◆ | Customize → MCPs tab (moved from the earlier Settings → Connections → MCP servers location); current plugin-based servers are the "MCP Marketplace", with legacy entries under a separate "Legacy MCP Marketplace"; custom stdio, SSE, and HTTP servers entered through a web form, gated by the "Manage MCP Servers" permission | No committed repo-level MCP file documented |
...
> **Powers (emerging, v2.21.0+, 2026-09-01):** Kiro shipped "Powers" — plugins bundling MCP servers, skills, and knowledge, built on an "Agent Plugins" spec co-maintained by Amazon/Cursor/Microsoft/OpenAI/Vercel. A power is `plugin.json` + `skills/` + optional `mcp.json` + optional `dev.kiro/`. Kiro's docs don't yet state an install/filesystem path (no documented `~/.kiro/powers/`), so it isn't added as a confident table row. `/config` (v2.21.0+) surfaces Powers as a peer category alongside Steering/Skills/Hooks/MCP/Agents. Current CLI version: v2.21.4 (2026-09-11).
...
| **MCP** ◆ | `~/.codeium/windsurf/mcp_config.json` (legacy Cascade path, still read); Devin Local does **not** add a separate user-level `mcp_config.json` — at user scope, MCP servers remain configured via the `mcpServers` key embedded in `~/.config/devin/config.json` (older entries auto-migrated in place); MCP Marketplace and enterprise allowlists | Only the **project** scope got a dedicated MCP file split (since the Local 3.6 / v3000.3 release, 2026-07-29; current Devin Local is v3000.6.14, 2026-09-03): `.devin/mcp_config.json` (team-shared, committed) and `.devin/mcp_config.local.json` (personal/gitignored) — superseding the earlier combined `.devin/config.json`/`.devin/config.local.json`. Note: the Desktop-specific docs page (`docs.devin.ai/desktop/devin-local`) hasn't been updated to reflect this and still only shows a generic `config.json`/`config.local.json` table — treat `docs.devin.ai/cli/extensibility/configuration` as authoritative for MCP-specific paths |
